...
def start():
    ...
    graphics.window.add(life_label, graphics.window.width-life_label.width-30, life_label.height + 20)
...
def serve(event):
    if 0 < event.x < graphics.window.width and 0 < event.y < graphics.window.height:
        ...
        if graphics.ball.x == (graphics.window_width - graphics.ball.width) / 2 and graphics.ball.y == (
