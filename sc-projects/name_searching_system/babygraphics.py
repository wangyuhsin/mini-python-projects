    ...
    for i in range(len(lookup_names)):
        ...
        for j in range(len(years)):
            if str(years[j]) not in name_data[lookup_names[i]]:
                canvas.create_text(get_x_coordinate(CANVAS_WIDTH, j)+TEXT_DX, CANVAS_HEIGHT-GRAPH_MARGIN_SIZE, text=lookup_names[i]+' *', anchor=tkinter.SW, fill=color)
                if j > 0:
                    canvas.create_line(x1, y1, get_x_coordinate(CANVAS_WIDTH, j), CANVAS_HEIGHT-GRAPH_MARGIN_SIZE, width=LINE_WIDTH, fill=color)
                x1 = get_x_coordinate(CANVAS_WIDTH, j)
                y1 = CANVAS_HEIGHT-GRAPH_MARGIN_SIZE
            else:
                rank = int(name_data[lookup_names[i]][str(years[j])])
                canvas.create_text(get_x_coordinate(CANVAS_WIDTH, j)+TEXT_DX, (CANVAS_HEIGHT-2*GRAPH_MARGIN_SIZE)/MAX_RANK*(rank-1)+GRAPH_MARGIN_SIZE, text=lookup_names[i]+' '+str(rank), anchor=tkinter.SW, fill=color)
                if j > 0:
                    canvas.create_line(x1, y1, get_x_coordinate(CANVAS_WIDTH, j), (CANVAS_HEIGHT-2*GRAPH_MARGIN_SIZE)/MAX_RANK*(rank-1)+GRAPH_MARGIN_SIZE, width=LINE_WIDTH, fill=color)
                x1 = get_x_coordinate(CANVAS_WIDTH, j)
                y1 = (CANVAS_HEIGHT-2*GRAPH_MARGIN_SIZE)/MAX_RANK*(rank-1)+GRAPH_MARGIN_SIZE
