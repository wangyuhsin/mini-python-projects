...
class SimpleImage(object):
    def __init__(self, filename, width=0, height=0, back_color=None):
        ...
        if filename:
            ...
            if width == 0 or height == 0:
                raise Exception(
                    "Creating blank image requires width/height but got {} {}".format(
                        width, height
                    )
                )
    ...
    def get_pixel(self, x, y):
        ...
        if x < 0 or x >= self._width or y < 0 or y >= self.height:
            e = Exception(
                "get_pixel bad coordinate x %d y %d (vs. image width %d height %d)"
    ...
    def set_pixel(self, x, y, pixel):
        if x < 0 or x >= self._width or y < 0 or y >= self.height:
            e = Exception(
                "set_pixel bad coordinate x %d y %d (vs. image width %d height %d)"
                % (x, y, self._width, self.height)
            )
            raise e
